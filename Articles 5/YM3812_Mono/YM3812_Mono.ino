...
now with primitive monophonic MIDI functionality. This code pairs with
an article and YouTube video whre we correct an issue with legato 
note playing.
...
void handleNoteOn( byte channel, byte midiNote, byte velocity ){               // Handle MIDI Note On Events
  PROC_YM3812.chPlayNote( 0, midiNote );                                       // Play midiNote on YM3812 channel 0
}

void handleNoteOff( byte channel, byte midiNote, byte velocity ){              // Handle MIDI Note Off Events
  PROC_YM3812.noteOff( midiNote );                                             // Turn off the midi note
}
