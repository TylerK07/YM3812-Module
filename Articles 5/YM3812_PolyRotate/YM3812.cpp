...
void YM3812::noteOn( uint8_t midiNote ){
  last_channel = (last_channel + 1) % num_channels;                            // Increment channel and rotate around number of channels
  channel_notes[ last_channel ] = midiNote;                                    // Store midi note associated with the channel
...
void YM3812::noteOff( uint8_t midiNote ){
  for( uint8_t ch = 0; ch<num_channels; ch++ ){
    if( channel_notes[ch] == midiNote ) regKeyOn( ch, 0 );                     // Turn off any channels associated with the midiNote
  }
}
